    ...
    try {
        switch (name) {
            case 'orchestrate_skills': {
                const result = await orchestrator.orchestrate(args.user_prompt, args.assistant_reasoning, args.project_context);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'analyze_context': {
                const context = orchestrator.analyzeContext(args.text);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'sync_skills_cache': {
                orchestrator.updateSkillsCache(args.skills);
                return {
                    content: [
                        {
                            type: 'text',
                            text: JSON.stringify({
                                success: true,
                                cached_count: args.skills.length,
            ...
            case 'record_skill_feedback': {
                orchestrator.recordFeedback(args.skill_name, args.outcome, args.feedback);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'search_invocation_history': {
                const results = orchestrator.searchHistory(args.query, args.limit);
                return {
                    content: [
                        {
                            type: 'text',
