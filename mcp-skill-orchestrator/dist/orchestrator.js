...
export class SkillOrchestrator {
     ...
     */
    async orchestrate(userPrompt, assistantReasoning, projectContext) {
        ...
        const matches = await this.skillMatcher.matchSkills(combinedContext, cachedSkills.length > 0 ? cachedSkills : undefined);
        // Categorize matches
        const { autoInvoke, suggestions } = this.skillMatcher.categorizeMatches(matches);
        // Generate reasoning
        const reasoning = this.generateOrchestrationReasoning(combinedContext, autoInvoke, suggestions);
     ...
     */
    recordFeedback(skillName, outcome, feedback) {
        const recent = this.db.getRecentSuccesses(1)[0];
        if (recent && recent.skill_name === skillName) {
            this.db.recordInvocation({
                ...
                invocationType: recent.invocation_type,
                outcome,
    ...
    getStatistics() {
        ...
        const successes = recentInvocations.filter((inv) => inv.outcome === 'success');
     ...
     */
    generateOrchestrationReasoning(context, autoInvoke, suggestions) {
        const parts = [];
        // Context summary
     ...
     */
    recordOrchestration(userPrompt, assistantReasoning, context, matches) {
        ...
        matches.forEach(match => {
            ...
            context.keywords.forEach(keyword => {
                this.db.updateKeywordMapping(keyword, match.skillName, match.confidenceScore);
            });
        });
    }
    /**
     ...
     */
    updatePatternLearning(invocation, outcome) {
