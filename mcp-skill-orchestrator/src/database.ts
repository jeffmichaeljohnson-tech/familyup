...
export interface SkillInvocation {
  ...
  invocationType: 'auto' | 'manual' | 'suggested';
...
export class SkillOrchestratorDB {
  ...
  recordInvocation(invocation: SkillInvocation) {
    ...
    return stmt.run(
      ...
      invocation.assistantReasoning,
      invocation.extractedKeywords,
      invocation.skillName,
      invocation.confidenceScore,
      invocation.invocationType,
      invocation.outcome,
      invocation.userFeedback
    );
  }
  ...
  learnPattern(pattern: ContextPattern) {
    const existing = this.db.prepare(
      'SELECT * FROM context_patterns WHERE pattern = ?'
    ).get(pattern.pattern) as ContextPattern | undefined;

    if (existing) {
      // Update success rate and invocation count
      const newSuccessRate =
        (existing.successRate * existing.invocationCount + pattern.successRate) /
  ...
  findMatchingPatterns(keywords: string[], limit = 10) {
  ...
  updateKeywordMapping(keyword: string, skillName: string, weight: number) {
    this.db.prepare(`
  ...
  getSkillsForKeywords(keywords: string[], limit = 5) {
  ...
  cacheSkillMetadata(metadata: SkillMetadata) {
    this.db.prepare(`
    ...
    `).run(
      ...
      metadata.lastInvoked
    );
  }

  // Get all cached skills
  getAllSkills() {
    const skills = this.db.prepare('SELECT * FROM skill_metadata').all() as any[];
