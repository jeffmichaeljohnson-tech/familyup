...
function findSkillsDir() {
    ...
    try {
        ...
        possiblePaths.push(path.join(__dirname, '..', '..', '.claude', 'Skills'), path.join(__dirname, '..', '..', '..', '.claude', 'Skills'));
    }
    catch {
 ...
 */
async function extractZipEntryText(zipfile, entry) {
 ...
 */
export async function loadSkillFromZip(zipPath) {
    return new Promise((resolve, reject) => {
...
export async function loadAllSkills() {
    try {
        ...
        const skills = await Promise.all(zipFiles.map(async (file) => {
            try {
