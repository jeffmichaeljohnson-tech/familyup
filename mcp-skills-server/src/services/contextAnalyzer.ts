...
import { ContextAnalysis, SkillRecommendation, SkillDocumentation, SkillSearchResult } from '../types/index.js';
...
export class ContextAnalyzer {
  ...
  analyzeContext(
    ...
    const recommendedSkills = this.findRelevantSkills(
      technicalDimension,
  ...
  private analyzeTechnicalDimension(
    query: string,
    code: string
  ): ContextAnalysis['technicalDimension'] {
    ...
    const hasComplexityIndicators = complexityIndicators.some(
      indicator => query.includes(indicator) || code.includes(indicator)
    );
  ...
  private analyzeArchitecturalDimension(
    ...
    code: string,
    projectType?: string
  ): ContextAnalysis['architecturalDimension'] {
    const patterns: string[] = [];
  ...
  private analyzePerformanceDimension(
    query: string,
    code: string
  ): ContextAnalysis['performanceDimension'] {
    const concerns: string[] = [];
  ...
  private findRelevantSkills(
    technical: ContextAnalysis['technicalDimension'],
    architectural: ContextAnalysis['architecturalDimension'],
    performance: ContextAnalysis['performanceDimension'],
    query: string
  ): SkillSearchResult[] {
