...
// Type definitions
type SkillSearchInput = z.infer<typeof SkillSearchInputSchema>;
type GetSkillInput = z.infer<typeof GetSkillInputSchema>;
type AnalyzeContextInput = z.infer<typeof AnalyzeContextInputSchema>;
...
Examples:
  ...
  - Get examples: name="react-development", format="examples"`,
    ...
    async (args: { name: string; format?: 'full' | 'metadata' | 'skill' | 'readme' | 'examples' }) => {
...
Examples:
  ...
  - List React-related skills: tags=["react"]`,
    ...
    async (args: { category?: string; tags?: string[]; limit?: number }) => {
...
Examples:
  ...
  - Analyze API design: query="Designing REST API for microservices", code_context="...code..."`,
    ...
    async (args: { query: string; code_context?: string; project_type?: string }) => {
      const analysis = analyzer.analyzeContext(
        args.query,
        args.code_context,
        args.project_type
      );

      const recommendations = analyzer.recommendSkills(
        args.query,
        args.code_context,
        args.project_type
      );
